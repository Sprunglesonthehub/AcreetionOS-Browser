...
#
# The script that patches the firefox source into the acreetionos_browser source.
#
...
def enter_srcdir(_dir = None):
    if _dir == None:
        dir = "acreetionos_browser-{}-{}".format(version, release)
    else:
        dir = _dir
    print("cd {}".format(dir))
    sys.stdout.flush()
...
def acreetionos_browser_patches():
    ...
    exec('mkdir -p lw')
    enter_srcdir('lw')
    exec('cp -v ../../settings/acreetionos_browser.cfg .')
    ...
    # 2) new files
    exec('cp ../patches/pref-pane/category-acreetionos_browser.svg browser/themes/shared/preferences/category-acreetionos_browser.svg')
    exec('cp ../patches/pref-pane/acreetionos_browser.css browser/themes/shared/preferences/acreetionos_browser.css')
    exec('cp ../patches/pref-pane/acreetionos_browser.inc.xhtml browser/components/preferences/acreetionos_browser.inc.xhtml')
    exec('cp ../patches/pref-pane/acreetionos_browser.js browser/components/preferences/acreetionos_browser.js')
...
#
# Main functionality in this script.. which is to call acreetionos_browser_patches()
#

if len(args) != 2:
    sys.stderr.write('error: please specify version and release of acreetionos_browser source')
    sys.exit(1)
version = args[0]
release = args[1]
srcdir = "acreetionos_browser-{}-{}".format(version, release)
if not os.path.exists(srcdir + '/configure.py'):
    sys.stderr.write('error: folder doesn\'t look like a Firefox folder.')
    sys.exit(1)

acreetionos_browser_patches()
